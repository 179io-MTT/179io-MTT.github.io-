...
function CurlHttp(url, method = 'GET', data = null, button = null) {
    if (button == null) {
        $.ajax({
            ...
            statusCode: {
                403: function () {
                    noti("error", "Ă„ÂĂ†Â°Ă¡Â»Âng dĂ¡ÂºÂ«n API khÄ‚Â´ng chÄ‚Â­nh xÄ‚Â¡c");
                },
                404: function () {
                    noti("error", "Ă„ÂĂ†Â°Ă¡Â»Âng dĂ¡ÂºÂ«n API khÄ‚Â´ng chÄ‚Â­nh xÄ‚Â¡c");
                },
            ...
            success: (data) => {
                if (!data && data == "undefined") {
                    noti("error", "KhÄ‚Â´ng thĂ¡Â»Æ’ lĂ¡ÂºÂ¥y dĂ¡Â»Â¯ liĂ¡Â»â€¡u");
                } else {
                    if (data.href) {
                        setInterval(() => {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                ...
                button
                    .prop("disabled", !0)
                    .html('<i class="fas fa-spinner fa-spin"></i> Ă„Âang tĂ¡ÂºÂ£i...');
            },
            complete: function () {
...
function copy(text) {
    ...
    noti('success', `Sao chĂ©p thĂ nh cĂ´ng <b>${text}</b>`)
}
$(function () {
